...
def test_h0_error_respects_bound():
    ...
    # Usa tolerancia numérica mínima para el factor 3.1
    assert H_0_ERROR <= PREDICTED_H0_ERROR * (1 + 1e-6), (
        f"H₀: {H_0_ERROR} supera la cota {PREDICTED_H0_ERROR} "
        f"(con tolerancia numérica 1e-6)"
    )
...
def test_all_errors_respect_structural_bounds():
    ...
    for name, measured_error, upper_bound, bound_formula in ERROR_BOUND_CASES:
        # Aplica tolerancia numérica solo para H₀ (factor 3.1)
        if name == "H₀":
            assert measured_error <= upper_bound * (1 + 1e-6)
        else:
